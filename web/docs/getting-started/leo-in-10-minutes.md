...
Finally, a **Log Window** [output channel](https://code.visualstudio.com/api/extension-capabilities/common-capabilities#output-channel) is present at the bottom of the window, showing text output from LeoJS commands, or from running scripts.
...
Leo is a [full-featured outliner](../users-guide/commands-reference.md#outline-commands), with commands to insert, delete, move, hoist, promote and demote nodes.
...
Leo has many other directives, described in the [directives reference page](../users-guide/directives-reference.md).

## External files

Leo outlines can refer to **external files**, files on your file system. Leo quickly loads the files when opening Leo outlines. The following sections discuss only the basics.  See [Leo's Reference Guide](../users-guide/directives-reference.md) for full details.
...
For more information, see Leo's [configuration guide](../users-guide/customizing-leo.md).
...
The execute-script command pre-defines three names: c, g, and p. **c** is the [commander](#accessing-outline-data) of the outline in which the script executes. **g** is the `leo.core.leoGlobals` module, containing dozens of useful functions and classes.  **p** is the [position](writing-scripts.md#positions-and-vnodes) of the presently selected node.
...
For more information, see Leo's [scripting tutorial](writing-scripts.md).
...
First, set the node's headline to: **@command print-tree**
 ...
 * with the number of characters in each node's body text.
 */
// p.self_and_subtree() is a python yielding p and
// all positions in p's subtree, in outline order.
for (const p of p.self_and_subtree()) {
    g.es(`${String(p.b.length).padStart(3)} ${' '.repeat(p.level())} ${p.h}`);
}
```
