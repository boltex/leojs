#@+leo-ver=5-thin
#@+node:felix.20230505220205.94: * @file src/importers/markdown.ts
...
#@+others
#@+node:felix.20230505220205.95: ** class Markdown_Importer(Importer)
class Markdown_Importer(Importer):
    ...
    #@+others
    #@+node:felix.20230505220205.96: *3* md_i.gen_lines & helpers
    ...
    #@+node:felix.20230505220205.97: *4* md_i.is_hash
    ...
    def is_hash(self, line: str) -> Tuple[int, str]:
        ...
        if m:
            ...
            if name:
                return level, name
        return None, None
    #@+node:felix.20230505220205.98: *4* md_i.is_underline
    md_pattern_table = (
        re.compile(r'^(=+)\n'),
        re.compile(r'^(-+)\n'),
    )
    ...
    def is_underline(self, line: str) -> bool:
        ...
        for pattern in self.md_pattern_table:
            ...
            if m and len(m.group(1)) >= 4:
                return True
        return False
    #@+node:felix.20230505220205.99: *4* md_i.lookahead_underline
    def lookahead_underline(self, i: int, lines: List[str]) -> bool:
        ...
        return False
    #@+node:felix.20230505220205.100: *4* md_i.make_decls_node
    ...
    #@+node:felix.20230505220205.101: *4* md_i.make_markdown_node
