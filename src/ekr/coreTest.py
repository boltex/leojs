...
#@@first
"""
Unit testing for leojs. Based on leoTest2.py.

Run these tests with unittest or pytest from the command line.

This file also contains two classes that convert nodes in unitTest.leo to
tests in leo/unittest.
"""
...
#@+others
#@+node:ekr.20210104121029.2: ** Top-level functions...
#@+node:ekr.20210104121029.3: *3* function: compareOutlines
def compareOutlines(root1, root2, compareHeadlines=True, tag='', report=True):
    ...
    return ok
#@+node:ekr.20210104121029.4: *3* function: convert_leoEditCommands_tests
...
#@+node:ekr.20210104121029.5: *3* function: create_app
def create_app():
    """
    ...
    if False and t4 - t3 > 0.1:
        print('create_app\n'
            ...
            f"    total: {(t4-t1):.3f}\n")
#@+node:ekr.20210104121029.6: *3* function: dump_leo_modules
def dump_leo_modules():
    ...
    for key in sorted(plugins):
        print(key)
#@+node:ekr.20210104121029.7: *3* function: expected_got
def expected_got(expected, got):
    ...
    for i, data in enumerate(zip(e_lines, g_lines)):
        ...
        result.append('\nDifferent lengths\n')
    return ''.join(result)
#@+node:ekr.20210104121029.8: *3* function: get_time
def get_time():
    return time.process_time()
#@+node:ekr.20210104121029.9: *3* function: showTwoBodies
def showTwoBodies(t, b1, b2):
    ...
    for line in splitLines(b2):
        print(f"{len(line):3d}", repr(line))
    print('-' * 20)
#@+node:ekr.20210104121029.10: *3* function: splitLines
def splitLines(s):
    """Same as g.splitLines(s)"""
    return s.splitlines(True) if s else []
#@+node:ekr.20210104121029.11: ** class ConvertTests
class ConvertTests:
    """
    ...
    """
    #@+others
    #@+node:ekr.20210104121029.12: *3* ConvertTests.body
    ...
    #@+node:ekr.20210104121029.13: *3* ConvertTests.class_name
    def class_name(self, command_name):
        ...
        return ''.join(result)
    #@+node:ekr.20210104121029.14: *3* ConvertTests.convert
    ...
    #@+node:ekr.20210104121029.15: *3* ConvertTests.function_name
    ...
    #@-others
#@+node:ekr.20210104121029.16: ** class ConvertEditCommandsTests (ConvertTests)
class ConvertEditCommandsTests (ConvertTests):
    
    #@+others
    #@+node:ekr.20210104121029.17: *3* ConvertEditCommandsTests.convert
    def convert(self, p, target):
