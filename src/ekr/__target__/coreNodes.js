...
import * as g from './coreGlobals.js';
var __name__ = 'coreNodes';
export var NodeIndices =  __class__ ('NodeIndices', [object], {
	...
	get _get_time () {return __get__ (this, function (self) {
...
export var Position =  __class__ ('Position', [object], {
	...
	get _relinkAsCloneOf () {return __get__ (this, function (self, p2) {
		...
		if (parent_v.children [p._childIndex] == v) {
			...
			g.error ('parent_v.children[childIndex] != v', p, parent_v.children, p._childIndex, v);
		}
	});},
			...
			if (child_v) {
				...
				for (var parent of p.self_and_parents (__kwargtrans__ ({copy: false}))) {
					if (child_v == parent.v) {
						...
						child_v.fileIndex = gNodeIndices.getNewIndex (__kwargtrans__ ({v: child_v}));
						p.moveToFirstChild ();
						__break0__ = true;
						break;
					}
...
export var VNode =  __class__ ('VNode', [object], {
	__module__: __name__,
	atAutoNames: set (['@auto-rst', '@auto']),
	atFileNames: set (['@asis', '@edit', '@file-asis', '@file-thin', '@file-nosent', '@file', '@clean', '@nosent', '@shadow', '@thin']),
	clonedBit: 1,
	expandedBit: 4,
	markedBit: 8,
	selectedBit: 32,
	...
	get anyAtFileNodeName () {return __get__ (this, function (self) {
		return self.findAtFileName (self.atAutoNames) || self.findAtFileName (self.atFileNames);
	});},
	get atAutoNodeName () {return __get__ (this, function (self, h) {
		if (typeof h == 'undefined' || (h != null && h.hasOwnProperty ("__kwargtrans__"))) {;
			var h = null;
		};
		return self.findAtFileName (self.atAutoNames, __kwargtrans__ ({h: h}));
	});},
		...
		if (__in__ (parent_v, v.parents)) {
			try {
				...
				if (isinstance (__except0__, ValueError)) {
					g.error ('{} not in parents of {}'.format (parent_v, v));
					g.trace ('v.parents:');
	...
	get _deleteAllChildren () {return __get__ (this, function (self) {
		...
		for (var v2 of v.children) {
			try {
				...
				if (isinstance (__except0__, ValueError)) {
					g.error ('{} not in parents of {}'.format (v, v2));
					g.trace ('v2.parents:');
...
export var gNodeIndices = NodeIndices ('ekr');

//# sourceMappingURL=coreNodes.map