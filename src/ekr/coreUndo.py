#@+leo-ver=5-thin
#@+node:ekr.20210109173916.1: * @file src/ekr/coreUndo.py
"""
...
class Undoer:
    ...
    def dumpTopBead(self):
        ...
        return '<no top bead>'
    #@+node:ekr.20210109201336.12: *4* u.getBead
    def getBead(self, n):
    ...
    def peekBead(self, n):
        ...
        if n < 0 or n >= len(u.beads):
            return None
        return u.beads[n]
    #@+node:ekr.20210109201336.14: *4* u.pushBead
    ...
    #@+node:ekr.20210109201336.21: *5* u.restoreVnodeUndoInfo
    def restoreVnodeUndoInfo(self, bunch):
        ...
        if uA is not None:
            ...
            v._p_changed = 1
    #@+node:ekr.20210109201336.22: *5* u.restoreTnodeUndoInfo
    def restoreTnodeUndoInfo(self, bunch):
        ...
        if uA is not None:
            v.unknownAttributes = uA
            v._p_changed = 1
    #@+node:ekr.20210109201336.23: *4* u.saveTree & helpers
    def saveTree(self, p, treeInfo=None):
        ...
        #@+<< about u.saveTree >>
        #@+node:ekr.20210109201336.24: *5* << about u.saveTree >>
        #@@language rest
        #@+at
        ...
        while child:
            ...
            child = child.next()
        return treeInfo
    #@+node:ekr.20210109201336.25: *5* u.createVnodeUndoInfo
    def createVnodeUndoInfo(self, v):
        ...
        return bunch
    #@+node:ekr.20210109201336.26: *5* u.createTnodeUndoInfo
    ...
    def updateMarks(self, oldOrNew):
        ...
        u.p.setDirty()
        u.c.setChanged()
    #@+node:ekr.20210109201336.29: *3* u.Externally visible entries
    #@+node:ekr.20210109201336.30: *4* u.afterX...
    #@+node:ekr.20210109201336.31: *5* u.afterChangeBody
    ...
    #@+node:ekr.20210109201336.32: *5* u.afterChangeGroup
    def afterChangeGroup(self, p, undoType, reportFlag=False):
        ...
        # Recalculate the menu labels.
        u.setUndoTypes()
    #@+node:ekr.20210109201336.33: *5* u.afterChangeNodeContents (deprecated)
    def afterChangeNodeContents(self, p, command, bunch):
        ...
        bunch.newYScroll = w.getYScrollPosition() if w else 0
        u.pushBead(bunch)
    #@+node:ekr.20210109201336.34: *5* u.afterChangeHeadline
    ...
    #@+node:ekr.20210109201336.35: *5* u.afterChangeTree
    ...
    def afterCloneMarkedNodes(self, p):
        ...
        bunch.newMarked = p.isMarked()
        u.pushBead(bunch)
    #@+node:ekr.20210109201336.38: *5* u.afterCopyMarkedNodes
    def afterCopyMarkedNodes(self, p):
        ...
        bunch.newMarked = p.isMarked()
        u.pushBead(bunch)
    #@+node:ekr.20210109201336.39: *5* u.afterCloneNode
    def afterCloneNode(self, p, command, bunch):
        ...
        bunch.newMarked = p.isMarked()
        u.pushBead(bunch)
    #@+node:ekr.20210109201336.40: *5* u.afterDehoist
    def afterDehoist(self, p, command):
        ...
        u.pushBead(bunch)
    #@+node:ekr.20210109201336.41: *5* u.afterDeleteNode
    def afterDeleteNode(self, p, command, bunch):
        ...
        bunch.newMarked = p.isMarked()
        u.pushBead(bunch)
    #@+node:ekr.20210109201336.42: *5* u.afterDeleteMarkedNodes
    def afterDeleteMarkedNodes(self, data, p):
        ...
        bunch.newMarked = p.isMarked()
        u.pushBead(bunch)
    #@+node:ekr.20210109201336.43: *5* u.afterDemote
    def afterDemote(self, p, followingSibs):
        ...
        # Recalculate the menu labels.
        u.setUndoTypes()
    #@+node:ekr.20210109201336.44: *5* u.afterHoist
    def afterHoist(self, p, command):
        ...
        u.pushBead(bunch)
    #@+node:ekr.20210109201336.45: *5* u.afterInsertNode
    def afterInsertNode(self, p, command, bunch):
        ...
        u.pushBead(bunch)
    #@+node:ekr.20210109201336.46: *5* u.afterMark
    def afterMark(self, p, command, bunch):
        ...
        bunch.newMarked = p.isMarked()
        u.pushBead(bunch)
    #@+node:ekr.20210109201336.47: *5* u.afterMoveNode
    def afterMoveNode(self, p, command, bunch):
        ...
        bunch.newP = p.copy()
        u.pushBead(bunch)
    #@+node:ekr.20210109201336.48: *5* u.afterPromote
    def afterPromote(self, p, children):
        ...
        # Recalculate the menu labels.
        u.setUndoTypes()
    #@+node:ekr.20210109201336.49: *5* u.afterSort
    def afterSort(self, p, bunch):
        ...
        # Recalculate the menu labels.
        u.setUndoTypes()
    #@+node:ekr.20210109201336.50: *4* u.beforeX...
    #@+node:ekr.20210109201336.51: *5* u.beforeChangeBody
    def beforeChangeBody(self, p):
        ...
        return bunch
    #@+node:ekr.20210109201336.52: *5* u.beforeChangeGroup
    def beforeChangeGroup(self, p, command, verboseUndoGroup=True):
        ...
        u.bead += 1
        u.beads[u.bead:] = [bunch]
    #@+node:ekr.20210109201336.53: *5* u.beforeChangeHeadline
    ...
    #@+node:ekr.20210109201336.54: *5* u.beforeChangeNodeContents (deprecated)
    def beforeChangeNodeContents(self, p):
        ...
        return bunch
    #@+node:ekr.20210109201336.55: *5* u.beforeChangeTree
    ...
    def beforeCloneNode(self, p):
        ...
        bunch = u.createCommonBunch(p)
        return bunch
    #@+node:ekr.20210109201336.58: *5* u.beforeDeleteNode
    def beforeDeleteNode(self, p):
        ...
        return bunch
    #@+node:ekr.20210109201336.59: *5* u.beforeInsertNode
    def beforeInsertNode(self, p, pasteAsClone=False, copiedBunchList=None):
        ...
        return bunch
    #@+node:ekr.20210109201336.60: *5* u.beforeMark
    def beforeMark(self, p, command):
        ...
        bunch.undoType = command
        return bunch
    #@+node:ekr.20210109201336.61: *5* u.beforeMoveNode
    def beforeMoveNode(self, p):
        ...
        return bunch
    #@+node:ekr.20210109201336.62: *5* u.beforeSort
    ...
    #@+node:ekr.20210109201336.65: *4* u.clearUndoState
    ...
    #@+node:ekr.20210109201336.66: *4* u.doTyping & helper
    def doTyping(self, p, undo_type, oldText, newText,
        ...
        **Important**: Code should call this method *only* when the user has
        ...
        #@+node:ekr.20210109201336.67: *5* << return if there is nothing to do >>
        if u.redoing or u.undoing:
            return None
        ...
        #@+node:ekr.20210109201336.68: *5* << init the undo params >>
        u.clearOptionalIvars()
        # Set the params.
        ...
        #@+node:ekr.20210109201336.69: *5* << compute leading, middle & trailing  lines >>
        ...
        #@-<< compute leading, middle & trailing  lines >>
        #@+<< save undo text info >>
        #@+node:ekr.20210109201336.70: *5* << save undo text info >>
        u.oldText = None
        u.newText = None
        u.leading = leading
        u.trailing = trailing
        ...
        #@+node:ekr.20210109201336.71: *5* << save the selection and scrolling position >>
        ...
        #@+node:ekr.20210109201336.72: *5* << adjust the undo stack, clearing all forward entries >>
        #@+at
        ...
        #@+node:ekr.20210109201336.73: *6* << set newBead if we can't share the previous bead >>
        ...
        if (
            ...
            if granularity == 'word' and not newBead:
                ...
                try:
                    ...
                    #@+node:ekr.20210109201336.74: *7* << set newBead if the change does not continue a word >>
    ...
    #@+node:ekr.20210109201336.75: *5* u.recognizeStartOfTypingWord
    ...
    #@+node:ekr.20210109201336.76: *4* u.enableMenuItems
    ...
    def updateAfterTyping(self, p, w):
        ...
        w.setFocus()
    #@+node:ekr.20210109201336.81: *3* u.redo
    @cmd('redo')
    def redo(self, event=None):
        ...
        u.bead += 1
        u.setUndoTypes()
    #@+node:ekr.20210109201336.82: *3* u.redo helpers
    #@+node:ekr.20210109201336.83: *4*  u.reloadHelper (do nothing)
    def redoHelper(self):
        """The default do-nothing redo helper."""
        pass
    #@+node:ekr.20210109201336.84: *4* u.redoChangeBody
    def redoChangeBody(self):
        ...
        u.updateMarks('new')
        u.p.setDirty()
    #@+node:ekr.20210109201336.85: *4* u.redoChangeHeadline
    ...
    def redoCloneMarkedNodes(self):
        ...
        c.cloneMarked()
        u.newP = c.p
    #@+node:ekr.20210109201336.88: *4* u.redoCopyMarkedNodes
    def redoCopyMarkedNodes(self):
        u = self; c = u.c
        c.selectPosition(u.p)
        c.copyMarked()
        u.newP = c.p
    #@+node:ekr.20210109201336.89: *4* u.redoCloneNode
    def redoCloneNode(self):
        ...
        u.newP.setDirty()
    #@+node:ekr.20210109201336.90: *4* u.redoDeleteMarkedNodes
    def redoDeleteMarkedNodes(self):
        u = self; c = u.c
        c.selectPosition(u.p)
        c.deleteMarked()
        c.selectPosition(u.newP)
    #@+node:ekr.20210109201336.91: *4* u.redoDeleteNode
    def redoDeleteNode(self):
        u = self; c = u.c
        c.selectPosition(u.p)
        c.deleteOutline()
        c.selectPosition(u.newP)
    #@+node:ekr.20210109201336.92: *4* u.redoDemote
    def redoDemote(self):
        u = self; c = u.c
    ...
    #@+node:ekr.20210109201336.93: *4* u.redoGroup
    def redoGroup(self):
    ...
    #@+node:ekr.20210109201336.94: *4* u.redoHoistNode & redoDehoistNode
    ...
    def redoDehoistNode(self):
        ...
        c.selectPosition(u.p)
        c.dehoist()
    #@+node:ekr.20210109201336.95: *4* u.redoInsertNode
    def redoInsertNode(self):
        ...
        u.newP.setDirty()
        c.selectPosition(u.newP)
    #@+node:ekr.20210109201336.96: *4* u.redoMark
    def redoMark(self):
        u = self; c = u.c
        u.updateMarks('new')
        if u.groupCount == 0:
            u.p.setDirty()
            c.selectPosition(u.p)
    #@+node:ekr.20210109201336.97: *4* u.redoMove
    def redoMove(self):
        u = self; c = u.c; cc = c.chapterController
        v = u.p.v
        ...
        u.newP.setDirty()
        c.selectPosition(u.newP)
    #@+node:ekr.20210109201336.98: *4* u.redoNodeContents
    def redoNodeContents(self):
        ...
        u.updateMarks('new')
        u.p.setDirty()
    #@+node:ekr.20210109201336.99: *4* u.redoPromote
    def redoPromote(self):
        ...
        u.p.setDirty()
        c.setCurrentPosition(u.p)
    #@+node:ekr.20210109201336.100: *4* u.redoSort
    def redoSort(self):
        u = self; c = u.c
        parent_v = u.p._parentVnode()
    ...
    #@+node:ekr.20210109201336.101: *4* u.redoTree
    def redoTree(self):
        ...
        if u.newSel:
            i, j = u.newSel
            c.frame.body.wrapper.setSelectionRange(i, j)
    #@+node:ekr.20210109201336.102: *4* u.redoTyping
    def redoTyping(self):
    ...
    #@+node:ekr.20210109201336.103: *3* u.undo
    @cmd('undo')
    def undo(self, event=None):
        ...
        u.bead -= 1
        u.setUndoTypes()
    #@+node:ekr.20210109201336.104: *3* u.undo helpers
    #@+node:ekr.20210109201336.105: *4*  u.undoHeoper
    def undoHelper(self):
        """The default do-nothing undo helper."""
        pass
    #@+node:ekr.20210109201336.106: *4* u.undoChangeBody
    def undoChangeBody(self):
        ...
        if u.groupCount == 0:
            ...
            c.frame.body.recolor(u.p)
        u.updateMarks('old')
    #@+node:ekr.20210109201336.107: *4* u.undoChangeHeadline
    ...
    #@+node:ekr.20210109201336.110: *4* u.undoCloneNode
    def undoCloneNode(self):
        ...
        u.p.setDirty()
        c.selectPosition(u.p)
    #@+node:ekr.20210109201336.111: *4* u.undoCopyMarkedNodes
    def undoCopyMarkedNodes(self):
        ...
        u.p.setAllAncestorAtFileNodesDirty()
        u.c.selectPosition(u.p)
    #@+node:ekr.20210109201336.112: *4* u.undoDeleteMarkedNodes
    def undoDeleteMarkedNodes(self):
        ...
        u.p.setAllAncestorAtFileNodesDirty()
        c.selectPosition(u.p)
    #@+node:ekr.20210109201336.113: *4* u.undoDeleteNode
    def undoDeleteNode(self):
        ...
        u.p.setDirty()
        c.selectPosition(u.p)
    #@+node:ekr.20210109201336.114: *4* u.undoDemote
    def undoDemote(self):
        u = self; c = u.c
        parent_v = u.p._parentVnode()
        ...
        u.p.setAllAncestorAtFileNodesDirty()
        c.setCurrentPosition(u.p)
    #@+node:ekr.20210109201336.115: *4* u.undoGroup
    def undoGroup(self):
        ...
        if oldSel:
            i, j = oldSel
            c.frame.body.wrapper.setSelectionRange(i, j)
    #@+node:ekr.20210109201336.116: *4* u.undoHoistNode & undoDehoistNode
    ...
    def undoDehoistNode(self):
        ...
        c.selectPosition(u.p)
        c.hoist()
    #@+node:ekr.20210109201336.117: *4* u.undoInsertNode
        ...
        if u.pasteAsClone:
            for bunch in u.beforeTree:
                ...
                if u.p.v == v:
                    ...
                    v.setBodyString(bunch.body)
                    v.setHeadString(bunch.head)
    #@+node:ekr.20210109201336.118: *4* u.undoMark
    def undoMark(self):
        u = self; c = u.c
        u.updateMarks('old')
        if u.groupCount == 0:
            u.p.setDirty()
            c.selectPosition(u.p)
    #@+node:ekr.20210109201336.119: *4* u.undoMove
    def undoMove(self):
        ...
        u.p.setDirty()
        c.selectPosition(u.p)
    #@+node:ekr.20210109201336.120: *4* u.undoNodeContents
    def undoNodeContents(self):
        ...
        if u.groupCount == 0 and u.oldYScroll is not None:
            w.setYScrollPosition(u.oldYScroll)
        u.updateMarks('old')
    #@+node:ekr.20210109201336.121: *4* u.undoPromote
    def undoPromote(self):
        ...
        u.p.setAllAncestorAtFileNodesDirty()
        c.setCurrentPosition(u.p)
    #@+node:ekr.20210109201336.122: *4* u.undoRedoText
    def undoRedoText(self, p,
        ...
        #@+node:ekr.20210109201336.123: *5* << Compute the result using p's body text >>
    ...
    #@+node:ekr.20210109201336.124: *4* u.undoRedoTree
    ...
    #@+node:ekr.20210109201336.125: *4* u.undoSort
    def undoSort(self):
        u = self; c = u.c
        parent_v = u.p._parentVnode()
        ...
        p.setAllAncestorAtFileNodesDirty()
        c.setCurrentPosition(p)
    #@+node:ekr.20210109201336.126: *4* u.undoTree
    def undoTree(self):
        """Redo replacement of an entire tree."""
        u = self; c = u.c
        ...
        if u.oldSel:
            i, j = u.oldSel
            c.frame.body.wrapper.setSelectionRange(i, j)
    #@+node:ekr.20210109201336.127: *4* u.undoTyping
    def undoTyping(self):
        c, u = self.c, self
        w = c.frame.body.wrapper
        # selectPosition causes recoloring, so don't do this unless needed.
