# -*- coding: utf-8 -*-
#@+leo-ver=5-thin
#@+node:ekr.20210101064206.1: * @file src/ekr/coreNodes.py
#@@first
...
class NodeIndices:
    ...
    #@+others
    #@+node:ekr.20210101064206.3: *3* ni._get_time (new)
    def _get_time(self):
        ...
        # return time.strftime(
            ...
            # time.localtime())
    #@+node:ekr.20210101064206.4: *3* ni.check_gnx
    def check_gnx(self, c, gnx, v):
        ...
        if v2 and v2 != v:
            g.error(
                ...
                f"          v: {v}\n"
                f"         v2: {v2}")
    #@+node:ekr.20210101064206.5: *3* ni.compute_last_index
            ...
            if gnx:
                ...
                if t == ni.timeString and n is not None:
                    try:
                        ...
                        g.es_exception()
                        self.lastIndex += 1
    #@+node:ekr.20210101064206.6: *3* ni.computeNewIndex
    def computeNewIndex(self):
        ...
        return gnx
    #@+node:ekr.20210101064206.7: *3* ni.get/setDefaultId
    ...
    #@+node:ekr.20210101064206.8: *3* ni.getNewIndex
    def getNewIndex(self, v, cached=False):
        ...
        **Important**: the method must allocate a new gnx even if v.fileIndex exists.
        ...
        fc.gnxDict[gnx] = v
        return gnx
    #@+node:ekr.20210101064206.9: *3* ni.new_vnode_helper
    ...
    #@+node:ekr.20210101064206.10: *3* ni.scanGnx
    def scanGnx(self, s, i=0):
        ...
        return theId, t, n
    #@+node:ekr.20210101064206.11: *3* ni.setTimeStamp
    def setTimeStamp(self):
        """Set the timestamp string to be used by getNewIndex until further notice"""
        self.timeString = self._get_time()
    #@+node:ekr.20210101064206.12: *3* ni.tupleToString
    ...
    #@+node:ekr.20210101064206.13: *3* ni.update
    def update(self):
        ...
        if self.timeString == t_s:
            ...
            self.timeString = t_s
        return t_s
    #@+node:ekr.20210101064206.14: *3* ni.updateLastIndex
    ...
    #@-others
#@+node:ekr.20210101064206.15: ** class Position
#@+<< about the position class >>
#@+node:ekr.20210101064206.16: *3* << about the position class >>
#@@language rest
#@+at
...
class Position:
    #@+others
    #@+node:ekr.20210101064206.17: *3*  p.ctor & other special methods...
    ...
    def __init__(self, v, childIndex=0, stack=None):
        ...
        if stack:
            ...
            self.stack = []
    #@+node:ekr.20210101064206.19: *4* p.__eq__ & __ne__
    def __eq__(self, p2):
    ...
    #@+node:ekr.20210101064206.20: *4* p.__ge__ & __le__& __lt__
    ...
    #@+node:ekr.20210101064206.21: *4* p.__gt__
    def __gt__(self, other):
        ...
        return x2 >= x1
    #@+node:ekr.20210101064206.22: *4* p.__nonzero__ & __bool__
    def __bool__(self):
        """
    ...
    #@+node:ekr.20210101064206.23: *4* p.__str__ and p.__repr__
    def __str__(self):
        p = self
        if p.v:
            return (
    ...
    __repr__ = __str__
    #@+node:ekr.20210101064206.24: *4* p.archivedPosition
    def archivedPosition(self, root_p=None):
        ...
        aList.reverse()
        return aList
    #@+node:ekr.20210101064206.25: *4* p.dump
    ...
    #@+node:ekr.20210101064206.26: *4* p.key & p.sort_key & __hash__
    def key(self):
        p = self
    ...
    __hash__ = None
    #@+node:ekr.20210101064206.27: *3* p.File Conversion
    #@+at
    # - convertTreeToString and moreHead can't be VNode methods because they uses level().
    # - moreBody could be anywhere: it may as well be a postion method.
    #@+node:ekr.20210101064206.28: *4* p.convertTreeToString
    def convertTreeToString(self):
        ...
        for p in p.self_and_subtree(copy=False):
            ...
            if body:
                array.append(body + '\n')
        return ''.join(array)
    #@+node:ekr.20210101064206.29: *4* p.moreHead
    ...
    #@+node:ekr.20210101064206.30: *4* p.moreBody
    #@@language rest
    #@+at
    #     + test line
    #     - test line
    ...
    def moreBody(self):
        ...
        for s in lines:
            ...
            array.append(s)
        return '\n'.join(array)
    #@+node:ekr.20210101064206.31: *3* p.generators
    #@+node:ekr.20210101064206.32: *4* p.children
    ...
    #@+node:ekr.20210101064206.33: *4* p.following_siblings
    ...
    #@+node:ekr.20210101064206.34: *4* p.nearest_roots
    def nearest_roots(self, copy=True, predicate=None):
        ...
        while p and p != after:
            if predicate(p):
                ...
                p.moveToThreadNext()
    #@+node:ekr.20210101064206.35: *4* p.nearest_unique_roots (aka p.nearest)
    ...
    #@+node:ekr.20210101064206.36: *4* p.nodes
    def nodes(self):
    ...
    tnodes_iter = nodes
    vnodes_iter = nodes
    #@+node:ekr.20210101064206.37: *4* p.parents
    ...
    #@+node:ekr.20210101064206.38: *4* p.self_and_parents
    ...
    #@+node:ekr.20210101064206.39: *4* p.self_and_siblings
    ...
    #@+node:ekr.20210101064206.40: *4* p.self_and_subtree
    ...
    #@+node:ekr.20210101064206.41: *4* p.subtree
    ...
    #@+node:ekr.20210101064206.42: *4* p.unique_nodes
    ...
    #@+node:ekr.20210101064206.43: *4* p.unique_subtree
    ...
    #@+node:ekr.20210101064206.44: *3* p.Getters
    #@+node:ekr.20210101064206.45: *4* p.VNode proxies
    #@+node:ekr.20210101064206.46: *5* p.Comparisons
    ...
    #@+node:ekr.20210101064206.47: *5* p.Headline & body strings
    def bodyString(self):
    ...
    #@+node:ekr.20210101064206.48: *5* p.Status bits
    ...
    #@+node:ekr.20210101064206.49: *4* p.children & parents
    #@+node:ekr.20210101064206.50: *5* p.childIndex
    # This used to be time-critical code.

    def childIndex(self):
        p = self
        return p._childIndex
    #@+node:ekr.20210101064206.51: *5* p.directParents
    def directParents(self):
        return self.v.directParents()
    #@+node:ekr.20210101064206.52: *5* p.hasChildren & p.numberOfChildren
    ...
    #@+node:ekr.20210101064206.53: *4* p.getX & VNode compatibility traversal routines
    ...
    hasVisBack = visBack
    hasVisNext = visNext
    #@+node:ekr.20210101064206.54: *4* p.get_UNL
    def get_UNL(
        self,
        with_file=True,
        with_proto=False,
        ...
        return UNL
    #@+node:ekr.20210101064206.55: *4* p.hasBack/Next/Parent/ThreadBack
    def hasBack(self):
        p = self
    ...
    #@+node:ekr.20210101064206.56: *5* hasThreadNext (the only complex hasX method)
    def hasThreadNext(self):
        ...
        while n >= 0:
            ...
            n -= 1
        return False
    #@+node:ekr.20210101064206.57: *4* p.findRootPosition
    def findRootPosition(self):
        # 2011/02/25: always use c.rootPosition
        p = self
        c = p.v.context
        return c.rootPosition()
    #@+node:ekr.20210101064206.58: *4* p.isAncestorOf
    def isAncestorOf(self, p2):
        ...
        for z in p2.stack:
            ...
            if parent_v == p.v and parent_childIndex == p._childIndex:
                return True
        return False
    #@+node:ekr.20210101064206.59: *4* p.isCloned
    def isCloned(self):
        p = self
        return p.v.isCloned()
    #@+node:ekr.20210101064206.60: *4* p.isRoot
    def isRoot(self):
        p = self
        return not p.hasParent() and not p.hasBack()
    #@+node:ekr.20210101064206.61: *4* p.isVisible (slow)
    def isVisible(self, c):
        ...
        for root in c.rootPosition().self_and_siblings(copy=False):
            if root == p or root.isAncestorOf(p):
                return visible(p)
        return False
    #@+node:ekr.20210101064206.62: *4* p.level & simpleLevel
    def level(self):
    ...
    simpleLevel = level
    #@+node:ekr.20210101064206.63: *4* p.positionAfterDeletedTree
    ...
    #@+node:ekr.20210101064206.64: *4* p.textOffset
    def textOffset(self):
        """
    ...
    #@+node:ekr.20210101064206.65: *3* p.isOutsideAtFileTree
    def isOutsideAnyAtFileTree(self):
        ...
        for parent in p.self_and_parents(copy=False):
            if parent.isAnyAtFileNode():
                return False
        return True
    #@+node:ekr.20210101064206.66: *3* p.Low level methods
    # These methods are only for the use of low-level code
    # in leoNodes.py, leoFileCommands.py and leoUndo.py.
    #@+node:ekr.20210101064206.67: *4* p._adjustPositionBeforeUnlink
    def _adjustPositionBeforeUnlink(self, p2):
        ...
        if changed:
            p.stack = stack
    #@+node:ekr.20210101064206.68: *4* p._linkAfter
    ...
    #@+node:ekr.20210101064206.69: *4* p._linkCopiedAfter
    ...
    #@+node:ekr.20210101064206.70: *4* p._linkAsNthChild
    def _linkAsNthChild(self, parent, n):
        ...
        child = p.v
        child._addLink(n, parent_v)
    #@+node:ekr.20210101064206.71: *4* p._linkCopiedAsNthChild
    def _linkCopiedAsNthChild(self, parent, n):
        ...
        child = p.v
        child._addCopiedLink(n, parent_v)
    #@+node:ekr.20210101064206.72: *4* p._linkAsRoot (changed)
    def _linkAsRoot(self):
        ...
        return p
    #@+node:ekr.20210101064206.73: *4* p._parentVnode
    def _parentVnode(self):
        ...
        return None
    #@+node:ekr.20210101064206.74: *4* p._relinkAsCloneOf
    ...
    #@+node:ekr.20210101064206.75: *4* p._unlink
    def _unlink(self):
    ...
    #@+node:ekr.20210101064206.76: *5* p.badUnlink
    ...
    #@+node:ekr.20210101064206.77: *3* p.moveToX
    ...
    #@+node:ekr.20210101064206.78: *4* p.moveToBack
    def moveToBack(self):
        ...
        if parent_v and p.v and 0 < n <= len(parent_v.children):
            ...
            p.v = None
        return p
    #@+node:ekr.20210101064206.79: *4* p.moveToFirstChild
    def moveToFirstChild(self):
        ...
        if p.v and p.v.children:
            ...
            p.v = None
        return p
    #@+node:ekr.20210101064206.80: *4* p.moveToLastChild
    def moveToLastChild(self):
        ...
        if p.v and p.v.children:
            ...
            p.v = None
        return p
    #@+node:ekr.20210101064206.81: *4* p.moveToLastNode
    def moveToLastNode(self):
        ...
        while p.hasChildren():
            p.moveToLastChild()
        return p
    #@+node:ekr.20210101064206.82: *4* p.moveToNext
    def moveToNext(self):
        ...
        if p.v and parent_v and len(parent_v.children) > n + 1:
            ...
            p.v = None
        return p
    #@+node:ekr.20210101064206.83: *4* p.moveToNodeAfterTree
    def moveToNodeAfterTree(self):
        ...
        while p:
            ...
            p.moveToParent()
        return p
    #@+node:ekr.20210101064206.84: *4* p.moveToNthChild
    def moveToNthChild(self, n):
        ...
        if p.v and len(p.v.children) > n:
            ...
            p.v = None
        return p
    #@+node:ekr.20210101064206.85: *4* p.moveToParent
    def moveToParent(self):
        ...
        if p.v and p.stack:
            ...
            p.v = None
        return p
    #@+node:ekr.20210101064206.86: *4* p.moveToThreadBack
    def moveToThreadBack(self):
        ...
        if p.hasBack():
            ...
            p.moveToParent()
        return p
    #@+node:ekr.20210101064206.87: *4* p.moveToThreadNext
    def moveToThreadNext(self):
        ...
        if p.v:
            if p.v.children:
                ...
                # not found.
        return p
    #@+node:ekr.20210101064206.88: *4* p.moveToVisBack & helper
        ...
        while p:
            ...
            if p:
                ...
                if p.isVisible(c):
                    return p
        return p
    #@+node:ekr.20210101064206.89: *5* checkVisBackLimit
    def checkVisBackLimit(self, limit, limitIsVisible, p):
        ...
        if limit.isAncestorOf(p):
            return False, None
        return True, None
    #@+node:ekr.20210101064206.90: *4* p.moveToVisNext & helper
        ...
        while p:
            ...
            if p:
                ...
                if p.isVisible(c):
                    return p
        return p
    #@+node:ekr.20210101064206.91: *5* checkVisNextLimit
    ...
    #@+node:ekr.20210101064206.92: *4* p.safeMoveToThreadNext
    def safeMoveToThreadNext(self):
        ...
        if p.v:
            ...
            if child_v:
                ...
                # not found.
        return p
    #@+node:ekr.20210101064206.93: *5* p.checkChild
    #@+node:ekr.20210101064206.94: *3* p.Moving, Inserting, Deleting, Cloning, Sorting
    #@+node:ekr.20210101064206.95: *4* p.clone
    def clone(self):
        ...
        return p2
    #@+node:ekr.20210101064206.96: *4* p.copy
    def copy(self):
    ...
    #@+node:ekr.20210101064206.101: *4* p.doDelete
    def doDelete(self, newNode=None):
        ...
        while sib.hasNext():
            ...
            if sib == newNode:
                ...
                break
        p._unlink()
    #@+node:ekr.20210101064206.102: *4* p.insertAfter
    def insertAfter(self):
        ...
        p2._linkAfter(p)
        return p2
    #@+node:ekr.20210101064206.103: *4* p.insertAsLastChild
    ...
    #@+node:ekr.20210101064206.104: *4* p.insertAsNthChild
    def insertAsNthChild(self, n):
        ...
        return p2
    #@+node:ekr.20210101064206.105: *4* p.insertBefore
    def insertBefore(self):
        ...
        if p.hasBack():
            ...
            p.moveToRoot()
        return p
    #@+node:ekr.20210101064206.106: *4* p.invalidOutline
    ...
    #@+node:ekr.20210101064206.107: *4* p.moveAfter
    def moveAfter(self, a):
        ...
        p._linkAfter(a)
        return p
    #@+node:ekr.20210101064206.108: *4* p.moveToFirst/LastChildOf
    ...
    #@+node:ekr.20210101064206.109: *4* p.moveToNthChildOf
    def moveToNthChildOf(self, parent, n):
        ...
        return p
    #@+node:ekr.20210101064206.110: *4* p.moveToRoot (changed)
    def moveToRoot(self):
        ...
        p._linkAsRoot()
        return p
    #@+node:ekr.20210101064206.111: *4* p.promote
    def promote(self):
    ...
    #@+node:ekr.20210101064206.112: *4* p.validateOutlineWithParent
    ...
    def validateOutlineWithParent(self, pv):
        ...
        #@+node:ekr.20210101064206.113: *5* << validate parent ivar >>
        if parent != pv:
            p.invalidOutline("Invalid parent link: " + repr(parent))
        #@-<< validate parent ivar >>
        #@+<< validate childIndex ivar >>
        #@+node:ekr.20210101064206.114: *5* << validate childIndex ivar >>
        if pv:
            if childIndex < 0:
        ...
        #@-<< validate childIndex ivar >>
        #@+<< validate x ivar >>
        #@+node:ekr.20210101064206.115: *5* << validate x ivar >>
        if not p.v and pv:
        ...
        return result
    #@+node:ekr.20210101064206.116: *3* p.Properties
    #@+node:ekr.20210101064206.117: *4* p.b property
    def __get_b(self):
    ...
    #@+node:ekr.20210101064206.118: *4* p.h property
    def __get_h(self):
        p = self
        return p.headString()
    ...
    #@+node:ekr.20210101064206.119: *4* p.gnx property
    def __get_gnx(self):
        p = self
    ...
    #@+node:ekr.20210101064206.120: *4* p.nosentinels property
    ...
    #@+node:ekr.20210101064206.121: *4* p.u Property
    def __get_u(self):
        p = self
        return p.v.u
    ...
    u = property(
        __get_u, __set_u,
        doc="p.u property")
    #@+node:ekr.20210101064206.122: *3* p.Setters
    #@+node:ekr.20210101064206.123: *4* p.VNode proxies
    #@+node:ekr.20210101064206.124: *5* p.contract/expand/isExpanded
    def contract(self):
    ...
    #@+node:ekr.20210101064206.125: *5* p.Status bits
    ...
    #@+node:ekr.20210101064206.126: *5* p.computeIcon & p.setIcon
    def computeIcon(self):
        return self.v.computeIcon()

    def setIcon(self):
        pass  # Compatibility routine for old scripts
    #@+node:ekr.20210101064206.127: *5* p.setSelection
    def setSelection(self, start, length):
        return self.v.setSelection(start, length)
    #@+node:ekr.20210101064206.128: *5* p.restore/saveCursorAndScroll
    def restoreCursorAndScroll(self):
        self.v.restoreCursorAndScroll()

    def saveCursorAndScroll(self):
        self.v.saveCursorAndScroll()
    #@+node:ekr.20210101064206.129: *4* p.setBodyString & setHeadString
    ...
    def setHeadString(self, s):
        ...
        p.v.initHeadString(s)
        p.setDirty()
    #@+node:ekr.20210101064206.130: *4* p.Visited bits
    #@+node:ekr.20210101064206.131: *5* p.clearVisitedInTree
    # Compatibility routine for scripts.

    def clearVisitedInTree(self):
        for p in self.self_and_subtree(copy=False):
            p.clearVisited()
    #@+node:ekr.20210101064206.132: *5* p.clearAllVisitedInTree
    def clearAllVisitedInTree(self):
        for p in self.self_and_subtree(copy=False):
            p.v.clearVisited()
            p.v.clearWriteBit()
    #@+node:ekr.20210101064206.133: *4* p.Dirty bits
    #@+node:ekr.20210101064206.134: *5* p.clearDirty
    def clearDirty(self):
        """(p) Set p.v dirty."""
        p = self
        p.v.clearDirty()
    #@+node:ekr.20210101064206.135: *5* p.inAtIgnoreRange
    def inAtIgnoreRange(self):
        ...
        for p in p.self_and_parents(copy=False):
            if p.isAtIgnoreNode():
                return True
        return False
    #@+node:ekr.20210101064206.136: *5* p.setAllAncestorAtFileNodesDirty
    ...
    #@+node:ekr.20210101064206.137: *5* p.setDirty
    def setDirty(self):
        """
        ...
        p.v.setAllAncestorAtFileNodesDirty()
        p.v.setDirty()
    #@+node:ekr.20210101064206.138: *3* p.Predicates
    #@+node:ekr.20210101064206.139: *4* p.is_at_all & is_at_all_tree
    def is_at_all(self):
    ...
    def in_at_all_tree(self):
        ...
        for p in p.self_and_parents(copy=False):
            if p.is_at_all():
                return True
        return False
    #@+node:ekr.20210101064206.140: *4* p.is_at_ignore & in_at_ignore_tree
...
#@+node:ekr.20210101064206.141: ** class VNode
#@@nobeautify
...
class VNode:
    ...
    #@+<< VNode constants >>
    #@+node:ekr.20210101064206.142: *3* << VNode constants >>
    ...
    __str__ = __repr__
    #@+node:ekr.20210101064206.146: *4* v.dump
    def dumpLink(self, link):
        return link if link else "<none>"
    ...
    #@+node:ekr.20210101064206.147: *3* v.Comparisons
    #@+node:ekr.20210101064206.148: *4* v.findAtFileName
    def findAtFileName(self, names, h=''):
        ...
        if word in names and g.match_word(h, 0, word):
            ...
            return name
        return ""
    #@+node:ekr.20210101064206.149: *4* v.anyAtFileNodeName
    ...
    #@+node:ekr.20210101064206.153: *4* v.isAt...FileNode
    ...
    isAtNoSentFileNode = isAtNoSentinelsFileNode
    isAtAsisFileNode = isAtSilentFileNode
    #@+node:ekr.20210101064206.154: *4* v.isAtIgnoreNode
    def isAtIgnoreNode(self):
        ...
        return flag
    #@+node:ekr.20210101064206.155: *4* v.isAtOthersNode
    def isAtOthersNode(self):
        """Returns True if the receiver contains @others in its body at the start of a line."""
        flag, i = g.is_special(self._bodyString, "@others")
        return flag
    #@+node:ekr.20210101064206.156: *4* v.matchHeadline
    ...
    #@+node:ekr.20210101064206.160: *4* v.Children
    #@+node:ekr.20210101064206.161: *5* v.firstChild
    def firstChild(self):
        v = self
        return v.children and v.children[0]
    #@+node:ekr.20210101064206.162: *5* v.hasChildren & hasFirstChild
    def hasChildren(self):
        v = self
        return len(v.children) > 0

    hasFirstChild = hasChildren
    #@+node:ekr.20210101064206.163: *5* v.lastChild
    def lastChild(self):
        v = self
        return v.children[-1] if v.children else None
    #@+node:ekr.20210101064206.164: *5* v.nthChild
    ...
    def nthChild(self, n):
        ...
        if 0 <= n < len(v.children):
            return v.children[n]
        return None
    #@+node:ekr.20210101064206.165: *5* v.numberOfChildren
    def numberOfChildren(self):
        v = self
        return len(v.children)
    #@+node:ekr.20210101064206.166: *4* v.directParents
    def directParents(self):
        ...
        v = self
        return v.parents
    #@+node:ekr.20210101064206.167: *4* v.hasBody
    def hasBody(self):
        """Return True if this VNode contains body text."""
        s = self._bodyString
        return s and len(s) > 0
    #@+node:ekr.20210101064206.168: *4* v.headString
    ...
    #@+node:ekr.20210101064206.169: *4* v.isNthChildOf
    ...
    #@+node:ekr.20210101064206.170: *4* v.Status Bits
    #@+node:ekr.20210101064206.171: *5* v.isCloned
    def isCloned(self):
        return len(self.parents) > 1
    #@+node:ekr.20210101064206.172: *5* v.isDirty
    def isDirty(self):
        return (self.statusBits & self.dirtyBit) != 0
    #@+node:ekr.20210101064206.173: *5* v.isMarked
    def isMarked(self):
        return (self.statusBits & VNode.markedBit) != 0
    #@+node:ekr.20210101064206.174: *5* v.isOrphan
    def isOrphan(self):
        return (self.statusBits & VNode.orphanBit) != 0
    #@+node:ekr.20210101064206.175: *5* v.isSelected
    def isSelected(self):
        return (self.statusBits & VNode.selectedBit) != 0
    #@+node:ekr.20210101064206.176: *5* v.isTopBitSet
    def isTopBitSet(self):
        return (self.statusBits & self.topBit) != 0
    #@+node:ekr.20210101064206.177: *5* v.isVisited
    def isVisited(self):
        return (self.statusBits & VNode.visitedBit) != 0
    #@+node:ekr.20210101064206.178: *5* v.isWriteBit
    def isWriteBit(self):
        v = self
        return (v.statusBits & v.writeBit) != 0
    #@+node:ekr.20210101064206.179: *5* v.status
    def status(self):
        return self.statusBits
    #@+node:ekr.20210101064206.180: *3* v.Setters
    #@+node:ekr.20210101064206.181: *4*  v.Dirty bits
    #@+node:ekr.20210101064206.182: *5* v.clearDirty
    def clearDirty(self):
        """Clear the vnode dirty bit."""
        v = self
        v.statusBits &= ~v.dirtyBit
    #@+node:ekr.20210101064206.183: *5* v.setDirty
    def setDirty(self):
        """
    ...
    #@+node:ekr.20210101064206.184: *4*  v.Status bits
    #@+node:ekr.20210101064206.185: *5* v.clearClonedBit
    def clearClonedBit(self):
        self.statusBits &= ~self.clonedBit
    #@+node:ekr.20210101064206.186: *5* v.clearMarked
    def clearMarked(self):
        self.statusBits &= ~self.markedBit
    #@+node:ekr.20210101064206.187: *5* v.clearWriteBit
    def clearWriteBit(self):
        self.statusBits &= ~self.writeBit
    #@+node:ekr.20210101064206.188: *5* v.clearOrphan
    def clearOrphan(self):

        self.statusBits &= ~self.orphanBit
    #@+node:ekr.20210101064206.189: *5* v.clearVisited
    def clearVisited(self):
        self.statusBits &= ~self.visitedBit
    #@+node:ekr.20210101064206.190: *5* v.contract/expand/initExpandedBit/isExpanded
    def contract(self):
        """Contract the node."""
    ...
    #@+node:ekr.20210101064206.191: *5* v.initStatus
    def initStatus(self, status):
        self.statusBits = status
    #@+node:ekr.20210101064206.192: *5* v.setClonedBit & initClonedBit
    ...
    def initClonedBit(self, val):
        if val:
            ...
            self.statusBits &= ~self.clonedBit
    #@+node:ekr.20210101064206.193: *5* v.setMarked & initMarkedBit
    def setMarked(self):
        self.statusBits |= self.markedBit

    def initMarkedBit(self):
        self.statusBits |= self.markedBit
    #@+node:ekr.20210101064206.194: *5* v.setOrphan
    def setOrphan(self):
        """Set the vnode's orphan bit."""
        self.statusBits |= self.orphanBit
    #@+node:ekr.20210101064206.195: *5* v.setSelected
    # This only sets the selected bit.

    def setSelected(self):
        self.statusBits |= self.selectedBit
    #@+node:ekr.20210101064206.196: *5* v.setVisited
    # Compatibility routine for scripts

    def setVisited(self):
        self.statusBits |= self.visitedBit
    #@+node:ekr.20210101064206.197: *5* v.setWriteBit
    def setWriteBit(self):
        self.statusBits |= self.writeBit
    #@+node:ekr.20210101064206.198: *4* v.computeIcon & setIcon
    def computeIcon(self):
        val = 0; v = self
        if v.hasBody(): val += 1
        if v.isMarked(): val += 2
    ...
    def setIcon(self):
        pass  # Compatibility routine for old scripts
    #@+node:ekr.20210101064206.199: *4* v.saveCursorAndScroll
    def saveCursorAndScroll(self):
        ...
        try:
            ...
            pass
    #@+node:ekr.20210101064206.200: *4* v.setBodyString & v.setHeadString
    ...
    setTnodeText = setBodyString
    #@+node:ekr.20210101064206.201: *4* v.setSelection
    def setSelection(self, start, length):
        v = self
        v.selectionStart = start
        v.selectionLength = length
    #@+node:ekr.20210101064206.202: *3* v.setAllAncestorAtFileNodesDirty
    def setAllAncestorAtFileNodesDirty(self):
        """
        ...
        for v2 in v_and_parents(v):
            if v2.isAnyAtFileNode():
                v2.setDirty()
    #@+node:ekr.20210101064206.203: *3* v.Inserting & cloning
    ...
    def insertAsNthChild(self, n):
        ...
        return v2
    #@+node:ekr.20210101064206.204: *3* v.Low level methods
    #@+node:ekr.20210101064206.205: *4* v._addCopiedLink
    ...
    def _addParentLinks(self, parent):
        ...
        if len(v.parents) == 1:
            for child in v.children:
                child._addParentLinks(parent=v)
    #@+node:ekr.20210101064206.208: *4* v._cutLink & _cutParentLinks
    ...
    def _cutParentLinks(self, parent):
        ...
        if not v.parents:
            for child in v.children:
                child._cutParentLinks(parent=v)
    #@+node:ekr.20210101064206.210: *4* v._deleteAllChildren
    def _deleteAllChildren(self):
        ...
        v.children = []
    #@+node:ekr.20210101064206.211: *4* v._linkAsNthChild
    def _linkAsNthChild(self, parent_v, n):
        """Links self as the n'th child of VNode pv"""
        v = self  # The child node.
        v._addLink(n, parent_v)
    #@+node:ekr.20210101064206.212: *3* v.Properties
    #@+node:ekr.20210101064206.213: *4* v.b Property
    def __get_b(self):
        v = self
        return v.bodyString()
    ...
    #@+node:ekr.20210101064206.214: *4* v.h property
    def __get_h(self):
        v = self
        return v.headString()
    ...
    #@+node:ekr.20210101064206.215: *4* v.u Property
    def __get_u(self):
        v = self
    ...
    #@+node:ekr.20210101064206.216: *4* v.gnx Property
    def __get_gnx(self):
        v = self
        return v.fileIndex
