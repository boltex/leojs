...
export class LeoUI extends NullGui {
    ...
    */
    ...
    public async selectTreeNode(
        ...
        // DoubleClick : First check if this got called recently (under 500ms) on the exact same node to signify a double click
        if (this.lastSelectedNode && this.lastSelectedNode.__eq__(p_node) && this.lastSelectedNodeTime) {
            if (utils.performanceNow() - this.lastSelectedNodeTime < 500) {
                // Double click on the same node
                this.lastSelectedNodeTime = undefined;
                isDoubleClick = true;
                if (!p_internalCall && g.doHook("icondclick1", { c: c, p: p_node, v: p_node })) {
